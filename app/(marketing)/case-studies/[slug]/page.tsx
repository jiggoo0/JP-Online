import { getPostBySlug, getAllPosts, CaseStudyFrontmatter } from "@/lib/mdx";
...
export async function generateStaticParams() {
  const posts = await getAllPosts<CaseStudyFrontmatter>("case-studies");
...
export async function generateMetadata({ params }: CaseStudyPageProps): Promise<Metadata> {
  ...
  const post = await getPostBySlug<CaseStudyFrontmatter>("case-studies", slug);
  if (!post) return {};
    ...
    <main className="min-h-screen bg-[#020617]">
      ...
      <div className="relative overflow-hidden border-b border-slate-900 bg-[#0c1122] pt-40 pb-20">
        ...
        <div className="relative z-10 container mx-auto max-w-4xl px-6">
          ...
          {/* Featured Case Visual Asset */}
          {post.data.image && (
            <div className="relative mt-24 aspect-video overflow-hidden border border-slate-900 shadow-2xl">
              <div
                className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-30 transition-all duration-1000 hover:opacity-50"
                style={{ backgroundImage: `url("${post.data.image}")` }}
              />
              <div className="absolute inset-0 bg-[linear-gradient(to_bottom,transparent_50%,rgba(12,17,34,0.8)_100%)]" />
      ...
      <Section className="relative mx-auto max-w-7xl px-6 py-24">
        ...
        <div className="mt-32">
          <ContactCTA
            title="ถอดบทเรียนสู่ความสำเร็จของคุณ"
            description="เราพร้อมนำประสบการณ์จากการแก้ปัญหาในเคสจริง มาปรับใช้เพื่อสร้างผลลัพธ์ที่ยอดเยี่ยมที่สุดให้กับคุณ"
          />
        </div>
      </Section>
    </main>
